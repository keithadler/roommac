...
#   ./make-dmg.sh            -> build/Room-for-Mac-<version>.dmg
...
DMG="build/Room-for-Mac-$VERSION.dmg"
STAGE=$(mktemp -d)
...
Room for Mac — how to install (no experience needed)

1. Drag the "Room for Mac" icon onto the "Applications" folder next to it.
...
4. Click the Apple menu (top-left) > System Settings > Privacy & Security.
   Scroll all the way down. Next to "Room for Mac was blocked", click
...
https://github.com/keithadler/roommac
TXT

rm -f "$DMG"
hdiutil create -volname "Room for Mac" -srcfolder "$STAGE" -ov -format UDZO -quiet "$DMG"
