...
4. **Open it the first time.** Open Launchpad (the rocket in the Dock) or your Applications folder and click
   **Room for Mac**. macOS will show a message saying it **can't check the app for malicious software** and
...
5. **Tell macOS it's okay, once.** Click the Apple menu at the top-left of the screen, then **System Settings**.
   ...
   saying "Room for Mac" was blocked, with an **Open Anyway** button. Click it, type your Mac password, and
   click **Open**.

6. **That's it.** From now on Room for Mac opens like any other app, and macOS never asks again. The first
...
```bash
xattr -d com.apple.quarantine "/Applications/Room for Mac.app"
```

Or build from source (Command Line Tools only, no Xcode), which also installs the `roommac` command and its man page:

```bash
git clone https://github.com/keithadler/roommac.git && cd roommac && ./build-app.sh --install --run
```
...
| Clean | Speed |
|---|---|
| ![Clean tab](docs/screenshots/clean.png) | ![Speed tab](docs/screenshots/speed.png) |

| Receipts | Uninstall an app |
|---|---|
| ![What Room for Mac did](docs/screenshots/receipts.png) | ![Uninstaller](docs/screenshots/uninstall.png) |

| Space map | Menu bar |
|---|---|
| ![Space map](docs/screenshots/spacemap.png) | ![Clean the menu bar](docs/screenshots/menubar.png) |
...
A second tab beside Clean, all without privileges:
...
- **Startup**: login items (System Events) with one-click removal; user launch agents with an on/off switch (plist parked under `Application Support/Room Mac/Disabled Agents`, receipt saved); all-user agents shown read-only.
...
- **Storage tricks**: the 10%-free guardrail (also drawn on the Clean gauge), purgeable space, deep links to Optimise Mac Storage, auto-empty Trash, Mail attachments.
...
Command line: `roommac speed` (add `--updates`, `--app-updates`, `--bench` for the slow checks). See [docs/CLI.md](docs/CLI.md).
...
- **Before you buy more storage**: free space plus what a clean would free, against iCloud tier prices
...
are filed by type into a `Sorted` folder right where they are. Moves, never trashes; the receipt
puts every file back exactly. Also `roommac sort`.

## Checkup report

The toolbar's document button (or `roommac report`) writes one HTML page: health verdict, safety,
what a clean would free, money findings, and what was done in the last 30 days. Made for handing to
whoever does the family's tech support.

## Menu bar window

Window > Clean the Menu Bar (⇧⌘B), also from the Speed tab and the sparkle menu. Toggles for Apple's icons (writes the same Control Center preferences System Settings does, then restarts Control Center), Spotlight, and Room for Mac's own icon; for other apps' icons: open their settings, stop them at login, or quit them. Reordering is ⌘-drag, explained in place. Points to Ice for collapsing icons since macOS has no built-in way.

## Extra windows

What Room for Mac did (receipts and put-back), Uninstall an App (drag an app in), Space Map (treemap of home), Disk Map (advanced). Settings: menu bar, launch at login, weekly quiet clean, reminders, protected folders. English and Spanish.
...
Room for Mac checks what this Mac can do at launch (`Capabilities.swift`) and never shows a control
it can't back up:
...
- If the Control Center preference layout isn't one Room for Mac recognises, the menu bar toggles are
...
`./make-dmg.sh` builds a universal, ad-hoc signed app and wraps it in `build/Room-for-Mac-<version>.dmg`
with an Applications shortcut and a first-open note. Upload it with
`gh release create v<version> build/Room-for-Mac-<version>.dmg`. With a Developer ID, sign with
`SIGN_IDENTITY` and run `notarize.sh` first so the first-open warning disappears.

## Build and run

```bash
./build-app.sh --run            # build/Room for Mac.app
./build-app.sh --install --run  # also copy to /Applications
```

## Command line

The same binary answers to `roommac` (not `clean`, which is too generic a name to take over on the PATH), installed as a symlink by `./build-app.sh --install` with a man page.
Every command shares the app's scanners and actions, and `--json` works everywhere.

```bash
roommac scan                          # what could be cleaned, changes nothing
roommac up --safe --dry-run           # the plan for the always-safe cards
roommac up --safe --yes               # do it; receipt saved
roommac speed                         # health verdict; exit 0 green, 1 yellow, 2 red
roommac speed --json --card safety    # one card, for scripts
roommac receipts && roommac restore last # undo
roommac disk                          # the boot-volume map
roommac money --speedtest --plan 300   # the Money tab, with an internet speed test
roommac sort --dry-run             # what the Desktop/Downloads sorter would file
roommac report --open              # the monthly checkup page
roommac screenshots docs/screenshots  # regenerate the README images from sample data
```

Full reference with exit codes and examples: [docs/CLI.md](docs/CLI.md), or `man roommac`.
...
```
Sources/RoomMac/
  RoomMacApp.swift    app entry: cleanup window, Disk Map window, text-mode hooks
...
icon/make-icon.swift  renders AppIcon.icns with AppKit (run: swift icon/make-icon.swift && iconutil -c icns icon/RoomMac.iconset -o AppIcon.icns)
```

## Privacy

Room for Mac makes no network requests on its own. The only time it talks to the internet is when you
...
is uploaded. Receipts and settings stay in `~/Library/Application Support/Room Mac`.
...
- **Notifications**: only if you turn on reminders or the weekly clean.
...
Mac and macOS are trademarks of Apple Inc., registered in the U.S. and other countries. Room for Mac
