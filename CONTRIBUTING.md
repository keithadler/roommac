# Contributing to Room for Mac

Thanks for looking. Room for Mac is small on purpose: one Swift package, no dependencies, no Xcode project.
...
2. **Every action leaves a receipt.** Anything moved gets a `CleanRecord` in a `CleanSession`
   (see `History.swift`), so "What Room for Mac did" can put it back.
...
```bash
.build/debug/RoomMac scan
.build/debug/RoomMac speed --all --bench --app-updates --updates
.build/debug/RoomMac disk
```
...
3. Add a color in `Room.color(for:)` in `CleanupView.swift`.
4. Add an alias in `CLI.scanAliases`, then run `roommac scan` and read the `[x]` lines. Anything pre-checked must survive rule 3 above.
...
4. Add the card to `CLI.speed` (text and `--json`) so it can be verified and scripted from the terminal, and document it in `docs/CLI.md` and `docs/roommac.1`.
