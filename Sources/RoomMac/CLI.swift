//  The command-line face of Room for Mac.
//
//  The same binary that runs the app also answers to `roommac <command>` (installed as a symlink in
...
enum CLI {
    ...
    /// When launched through the `roommac` symlink, Bundle.main doesn't know about the .app, so
    ...
    static let usage = """
    roommac — a friendly, safe cleanup and speed-up for your Mac (command-line face)

    USAGE
      roommac scan [--card <name>…] [--json]
      roommac up (--safe | --card <name>… | --all-checked) [--dry-run] [--yes] [--json]
      roommac speed [--card <name>…] [--all] [--updates] [--app-updates] [--bench] [--json]
      roommac receipts [--json]
      roommac restore <session-id | last> [--json]
      roommac trash [--empty --yes] [--json]
      roommac disk [--json]
      roommac money [--speedtest] [--plan <mbps>] [--json]
      roommac sort [--desktop] [--downloads] [--older-than <days>] [--dry-run] [--yes] [--json]
      roommac report [--out <file>] [--md] [--open]
      roommac screenshots <directory>
      roommac help | version

    COMMANDS
      scan        Show what could be cleaned, card by card. Nothing is changed.
      up          Move the pre-checked items of the chosen cards to the Trash and save a receipt.
                  --safe is the same set the weekly quiet clean uses (caches, logs, installers,
      ...
      sort        File loose Desktop/Downloads files older than N days (default 7) into typed
                  folders in a "Sorted" folder. Moves, never deletes; receipt saved; --dry-run
      ...
      report      The monthly checkup as one page. HTML by default, --md for Markdown, --out to
                  choose the file (default ~/Desktop/Mac checkup <date>.html), --open to view it.
    ...
    a receipt you can undo with `roommac restore`. See docs/CLI.md for details and examples.
    """
    ...
    static func runIfRequested() {
        ...
        case "version", "--version", "-v": print("roommac \(version) (Room for Mac)"); code = 0
        ...
        default:
            ...
            fputs("roommac: unknown command '\(cmd)'\n\n\(usage)\n", stderr); code = 64
        }
        exit(code)
    }
    ...
    static func fail(_ msg: String, json: Bool) -> Int32 {
        if json { emit(["error": msg]) } else { fputs("roommac: \(msg)\n", stderr) }
        return 70
    }
    ...
    static func scanKinds(_ opts: Options) -> [CleanKind]? {
        ...
        for c in opts.cards {
            guard let k = scanAliases[c.lowercased()] ?? CleanKind(rawValue: c) else {
                fputs("roommac: unknown card '\(c)'. Cards: \(scanAliases.keys.sorted().joined(separator: " "))\n", stderr)
                return nil
            }
    ...
    static func scan(_ opts: Options) -> Int32 {
        ...
        print()
        if cats.isEmpty { print("Sparkling clean. Nothing to clean."); return 0 }
        for c in cats {
        ...
        print("Pre-checked: \(Bytes.string(checked)), \(Bytes.friendly(checked)). Run `roommac up --safe` (or --card …) to move them to the Trash.")
        return 0
    }
    ...
    static func up(_ opts: Options) -> Int32 {
        ...
        if opts.flags.contains("--safe") { kinds = CleanKind.quiet }
            ...
            fputs("roommac up: say what to clean: --safe, --card <name>…, or --all-checked. Nothing was changed.\n", stderr)
            return 64
        }
        ...
        for i in items {
            ...
            for p in i.paths {
                ...
                records.append(CleanRecord(id: UUID(), name: i.name, originalPath: p, trashPath: t, size: per, restored: false, kind: kindOf[i.id]))
            }
        }
        let session = CleanSession(id: UUID(), date: Date(), automatic: false, label: "roommac up (command line)", records: records)
        ...
        if opts.json {
            ...
            print("Receipt \(String(session.id.uuidString.prefix(8))) saved. `roommac restore last` puts everything back while it's still in the Trash.")
        }
    ...
    static func speed(_ opts: Options) -> Int32 {
        ...
        if opts.cards.isEmpty {
            ...
            if let bad = wanted.first(where: { !speedCards.contains($0) }) {
                fputs("roommac speed: unknown card '\(bad)'. Cards: \(speedCards.joined(separator: " "))\n", stderr); return 64
            }
        }
        ...
        return MainActor.assumeIsolated { () -> Int32 in
            ...
            if json {
                ...
                if wanted.contains("startup") { cards["startup"] = ["loginItems": m.loginItems.map { ["name": $0.name, "path": $0.path] }, "error": m.loginItemsError as Any,
                                                                     "agents": m.agents.map { ["label": $0.label, "vendor": $0.vendor, "loaded": $0.loaded, "system": $0.system, "disabled": $0.disabledByClean, "memoryBytes": $0.memory, "cpuPercent": $0.cpu] }] }
            ...
            if wanted.contains("startup") {
                ...
                for a in m.agents where !a.system { print("   helper: \(a.vendor) · \(a.label) · \(a.disabledByClean ? "off" : (a.loaded ? "loaded" : "not running")) · \(Bytes.string(a.memory))") }
            }
    ...
    static func receipts(_ opts: Options) -> Int32 {
        MainActor.assumeIsolated {
            ...
            if opts.json {
                emit(sessions.map { s in ["id": s.id.uuidString, "date": iso(s.date) as Any, "label": s.label ?? (s.automatic ? "quiet clean" : "roommac up"), "automatic": s.automatic,
            ...
            for s in sessions {
                print("\(s.id.uuidString.prefix(8))  \(s.date.formatted(date: .abbreviated, time: .shortened))  \(pad(s.label ?? (s.automatic ? "quiet clean" : "roommac up"), 32)) \(s.movedCount) items · \(Bytes.string(s.totalSize))\(s.restorableCount > 0 ? " · \(s.restorableCount) can be put back" : "")")
            }
            print("\n`roommac restore <id>` or `roommac restore last` puts a session back while it's still in the Trash.")
            return 0
        }
    }

    static func restore(_ opts: Options) -> Int32 {
        guard let which = opts.positional.first else { fputs("roommac restore: give a session id (see `roommac receipts`) or `last`.\n", stderr); return 64 }
    ...
    static func money(_ opts: Options) -> Int32 {
        return MainActor.assumeIsolated { () -> Int32 in
            ...
            print("\n== Subscription apps (Room can't see your bills; these are questions)")
            ...
            if let sp = storage.space {
                print("\n== Storage: \(Bytes.string(sp.free)) free · a clean frees \(Bytes.string(storage.safeBytes)) safely + \(Bytes.string(storage.reviewBytes)) to review\(storage.icloudLocalBytes.map { " · iCloud Drive keeps \(Bytes.string($0)) locally" } ?? "")")
            }
            if let s = speed {
    ...
    static func sort(_ opts: Options) -> Int32 {
        ...
        if !opts.yes {
            print("File \(moves.count) files (\(Bytes.string(total))) into Sorted folders by type?")
            for c in Sorter.categories { let ms = moves.filter { $0.category == c }; if !ms.isEmpty { print("   \(pad(c, 12)) \(ms.count) files") } }
            if !confirm("Continue?") { print("Nothing was changed."); return 3 }
        }
        ...
        else { print("Filed \(session.movedCount) files into Sorted folders.\(session.failedCount > 0 ? " \(session.failedCount) couldn't be moved." : "") `roommac restore last` puts them back.") }
        return 0
    }
    ...
    static func report(_ opts: Options) -> Int32 {
        ...
        let defaultName = "Mac checkup \(Date().formatted(date: .numeric, time: .omitted).replacingOccurrences(of: "/", with: "-")).\(md ? "md" : "html")"
    ...
    static func trash(_ opts: Options) -> Int32 {
        ...
        else { print(t.readable ? "Trash: \(t.count) items, \(Bytes.string(t.size)). `roommac trash --empty` removes them for good." : "Trash size isn't readable without Full Disk Access. `roommac trash --empty` still works.") }
        return 0
    }
