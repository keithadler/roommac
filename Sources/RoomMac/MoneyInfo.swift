...
enum MoneyInfo {
    ...
    static let subscriptionCatalog: [(match: [String], vendor: String, manage: String, note: String)] = [
        ...
        (["com.macpaw.cleanmymac", "com.macpaw.CleanMyMac"], "CleanMyMac", "https://my.macpaw.com", "Cleaner subscription. Room for Mac does this for free."),
        (["com.kromtech.", "com.mackeeper."], "MacKeeper", "https://account.mackeeper.com", "Cleaner subscription. Room for Mac does this for free."),
    ...
    static func macVerdict(hw: HardwareInfo, health: HealthReport?, memory: MemoryInfo?, space: DiskSpace?) -> MacVerdict {
        ...
        if let s = space {
            ...
            if freeFrac < 0.1 { score += 1; reasons.append(String(localized: "The disk is nearly full, which makes any Mac feel old.")); fixes.append(String(localized: "run Clean Up, then move big files to an external SSD (about $\(Int(externalSSDPerTB)) per TB)")) }
        }
        if let h = health {
    ...
    static let alternativesCatalog: [(match: [String], builtIn: String, note: String, open: String?)] = [
        ...
        (["com.macpaw.cleanmymac", "com.kromtech", "com.mackeeper", "com.piriform.ccleaner", "com.nektony"], "Room for Mac", "You're looking at it. Same job, free, and it never deletes.", nil),
    ...
    static func builtInAlternatives() -> [BuiltInAlternative] {
        ...
        for u in Scanner.contents(URL(fileURLWithPath: "/Applications")) where u.pathExtension == "app" {
            guard let bid = Bundle(url: u)?.bundleIdentifier?.lowercased() else { continue }
            if bid == "com.keithadler.roommac" { continue }
