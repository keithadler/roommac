//  The main window's content: a segmented Clean | Speed switch in the toolbar and the
//  matching view below it. The selected tab is remembered in UserDefaults ("mainTab").

import SwiftUI

/// The main window: Clean (cleanup cards) and Speed (health and performance) side by side.
struct MainView: View {
        ...
        .toolbar {
            ToolbarItem(placement: .principal) {
                Picker("Section", selection: $tab) {
                    Label("Clean", systemImage: "sparkles").tag(0)
                    Label("Speed", systemImage: "gauge.with.dots.needle.67percent").tag(1)
                    Label("Money", systemImage: "dollarsign.circle").tag(2)
                }
                .pickerStyle(.segmented)
                .labelStyle(.titleAndIcon)
                .accessibilityLabel(Text("Section: Clean, Speed, or Money"))
            }
            ToolbarItem(placement: .navigation) {
                Button { saveReport() } label: { Label("Save checkup report", systemImage: reporting ? "hourglass" : "doc.text") }
                    .disabled(reporting)
                    .help("One page with the health verdict, what a clean would free, and the money findings. Good for handing to whoever helps with the Mac.")
            }
        }
    }
    ...
    private func saveReport() {
        ...
        panel.nameFieldStringValue = "Mac checkup \(Date().formatted(date: .numeric, time: .omitted).replacingOccurrences(of: "/", with: "-")).html"
