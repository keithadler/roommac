//  The monthly checkup report: one page with the health verdict, what a clean would free, the money
...
enum Report {
    ...
    static func markdown(_ r: ReportData) -> String {
        ...
        for x in r.sessions { s += "- \(x.date.formatted(date: .abbreviated, time: .shortened)) · \(x.label ?? (x.automatic ? "quiet clean" : "clean up")) · \(x.movedCount) items · \(Bytes.string(x.totalSize))\n" }
        s += "\n---\n_Made by Room for Mac. Nothing in this report was changed on the Mac; everything Room moves goes to the Trash with a receipt._\n"
        return s
    }
    ...
    static func html(_ r: ReportData) -> String {
        ...
        return """
        <!doctype html><html><head><meta charset="utf-8"><title>Room for Mac checkup</title>
