//  Settings window: General (menu bar, launch at login, weekly clean, reminders), Protected
...
struct GeneralSettings: View {
    ...
    @AppStorage(Scheduler.Key.weeklyClean) private var weeklyClean = false
    ...
    var body: some View {
        Form {
            Section {
                Toggle("Show Room for Mac in the menu bar", isOn: $menuBar)
                Toggle("Open Room for Mac when I log in", isOn: $launchAtLogin)
                ...
                if let e = loginError { Text(e).font(.caption).foregroundStyle(Room.red) }
                Text("Together these keep the sparkle in the menu bar so reminders and the weekly clean can run.")
                    .font(.caption).foregroundStyle(.secondary)
            }
            Section {
                Toggle("Clean the safe things every week", isOn: $weeklyClean)
                    .onChange(of: weeklyClean) { _, on in if on { askForNotifications() } }
                Text("Only app caches, logs, old installers, and developer caches. Never anything from a \"take a look first\" card. A receipt is saved every time.")
                    .font(.caption).foregroundStyle(.secondary)
                Toggle("Remind me when it's time to clean, or when the disk is almost full", isOn: $reminders)
                    .onChange(of: reminders) { _, on in if on { askForNotifications() } }
                if notifDenied {
                    Text("Notifications are turned off for Room for Mac in System Settings > Notifications, so reminders will be quiet.")
                        .font(.caption).foregroundStyle(Room.orange)
                }
            }
            Section {
                Text("Language follows System Settings > General > Language & Region. Room for Mac speaks English and Spanish.")
                    .font(.caption).foregroundStyle(.secondary)
            }
        }
        .formStyle(.grouped)
...
struct ProtectedSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Folders Room for Mac must never touch").font(.headline)
            ...
            List(selection: $selection) {
                ForEach(paths, id: \.self) { p in
                    HStack {
                        Image(systemName: "folder.fill").foregroundStyle(Room.blue).accessibilityHidden(true)
...
struct AboutSettings: View {
    var body: some View {
        VStack(spacing: 12) {
            ...
            Text("Room for Mac").font(.title.weight(.bold))
