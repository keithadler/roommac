...
struct NetworkCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: Capabilities.hasWiFi ? "Wi-Fi and internet" : "Internet"), icon: "wifi", color: Room.blue,
            ...
            if let n = model.network {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if !n.fasterBandAvailable, (n.signal ?? -100) >= -70, (n.dnsMillis ?? 0) <= 200, (n.gatewayMillis ?? 0) <= 60, n.signal != nil {
                        tip(Room.green, "Nothing to fix here.")
                    }
                }
                HStack {
...
struct SyncCard: View {
    ...
    var body: some View {
        ...
        if !present.isEmpty {
            SpeedCard(title: String(localized: "Cloud sync"), icon: "arrow.triangle.2.circlepath.icloud", color: Room.teal,
                ...
                ForEach(present) { s in
                    HStack(spacing: 10) {
                        Image(systemName: s.stuck ? "exclamationmark.arrow.triangle.2.circlepath" : (s.running ? "checkmark.icloud" : "icloud.slash"))
                            .foregroundStyle(s.stuck ? Room.orange : (s.running ? Room.green : .secondary))
                        VStack(alignment: .leading, spacing: 1) {
...
struct TimeMachineCard: View {
    ...
    var body: some View {
        ...
        SpeedCard(title: String(localized: "Time Machine"), icon: "clock.arrow.circlepath", color: Room.purple,
            ...
            if let t = model.timeMachine {
                if !t.configured {
                    tip(Room.orange, "Time Machine isn't set up. A cheap external drive is the best insurance a Mac can have.")
                } else {
                    HStack(spacing: 8) {
                        Image(systemName: "externaldrive.badge.timemachine").foregroundStyle(Room.purple)
                    ...
                    if let last = t.lastBackup {
                        ...
                        tip(days > 7 ? Room.orange : Room.green, days > 7
                ...
                if !t.localSnapshots.isEmpty {
                    ...
                    tip(t.localSnapshots.count > 6 ? Room.orange : .secondary,
...
struct AppUpdatesCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Apps that update themselves"), icon: "arrow.triangle.2.circlepath", color: Room.orange,
            ...
            if model.checkingAppUpdates {
                ...
                if outdated.isEmpty {
                    Label("Everything we could check is current", systemImage: "checkmark.circle.fill").foregroundStyle(Room.green)
                }
                ForEach(outdated) { a in
                    HStack(spacing: 10) {
                        ...
                        Text("\(a.installed) → \(a.latest ?? "?")").font(.caption.monospacedDigit()).foregroundStyle(Room.orange)
                        Spacer()
...
struct RosettaCard: View {
    ...
    var body: some View {
        if Capabilities.isAppleSilicon {
            SpeedCard(title: String(localized: "Apps built for old Intel Macs"), icon: "cpu", color: Room.slate,
                ...
                if let list = model.rosettaApps {
                    if list.isEmpty {
                        Label("Every app in your Applications folder is native. Nice.", systemImage: "checkmark.circle.fill").foregroundStyle(Room.green)
                    } else {
                        ForEach(list) { a in
                            HStack(spacing: 10) {
                                Image(nsImage: NSWorkspace.shared.icon(forFile: a.path)).resizable().frame(width: 22, height: 22)
...
struct MenuBarCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Menu bar apps and widgets"), icon: "menubar.rectangle", color: Room.pink,
            ...
            if model.menuBarApps.isEmpty && model.widgets.isEmpty {
                Label("Nothing extra running in the menu bar.", systemImage: "checkmark.circle.fill").foregroundStyle(Room.green)
            }
            HStack {
...
struct PrinterCard: View {
    ...
    var body: some View {
        if let list = model.printers, !list.isEmpty {
            SpeedCard(title: String(localized: "Printers"), icon: "printer", color: Room.brown,
                ...
                ForEach(list) { p in
                    HStack(spacing: 10) {
                        Image(systemName: p.state == "disabled" ? "printer.fill.and.paper.fill" : "printer.fill")
                            .foregroundStyle(p.state == "disabled" || p.state == "paused" ? Room.orange : Room.green)
                        VStack(alignment: .leading, spacing: 1) {
                            HStack(spacing: 6) {
...
struct PowerCard: View {
    ...
    var body: some View {
        if let p = model.power, p.percent != nil {
            SpeedCard(title: String(localized: "Battery habits"), icon: "battery.75percent", color: Room.green,
                ...
                VStack(alignment: .leading, spacing: 4) {
                    if let h = model.health?.battery, let pct = h.healthPercent, pct < 85 {
                        tip(Room.orange, "Battery holds \(pct)% of new. Keep it between 20% and 80% when you can, and avoid leaving it at 100% on the charger for days.")
                    }
...
struct SleepCard: View {
    ...
    var body: some View {
        ...
        SpeedCard(title: String(localized: "Sleep and wake"), icon: "moon.zzz.fill", color: Room.purple,
            ...
            if let s = model.sleep {
                if !s.assertions.isEmpty {
                    ...
                    ForEach(s.assertions) { a in
                        HStack(alignment: .top, spacing: 8) {
                            Image(systemName: "eye").foregroundStyle(Room.orange).padding(.top, 2)
                            VStack(alignment: .leading, spacing: 1) {
                    ...
                    Label("Nothing is stopping the Mac from sleeping.", systemImage: "checkmark.circle.fill").foregroundStyle(Room.green)
                }
    ...
    private func setting(_ name: LocalizedStringKey, _ on: Bool?, _ note: LocalizedStringKey) -> some View {
        HStack(alignment: .top, spacing: 8) {
            Image(systemName: on == true ? "checkmark.circle.fill" : "circle").foregroundStyle(on == true ? Room.orange : .secondary).padding(.top, 2)
            VStack(alignment: .leading, spacing: 1) {
...
struct DiskSpeedCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "How fast are your drives?"), icon: "speedometer", color: Room.red,
            ...
            ForEach(model.volumes) { v in
                HStack(spacing: 10) {
                    Image(systemName: v.isInternal ? "internaldrive" : "externaldrive").foregroundStyle(v.isInternal ? Room.blue : Room.orange)
                    VStack(alignment: .leading, spacing: 1) {
