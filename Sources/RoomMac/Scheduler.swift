//  Background behaviour: weekly quiet clean, "almost full" warnings, monthly nudges, and
...
/// Weekly quiet clean, "almost full" warnings, and gentle nudges. Runs while the app is open,
...
final class Scheduler {
    ...
    enum Key {
        static let weeklyClean = "weeklyClean"
    ...
    /// fires at most once per period even if the app is relaunched repeatedly. The weekly clean
    /// stamps *before* running so a crash mid-clean can't cause a loop of retries.
    func check(_ model: CleanupModel) {
        if defaults.bool(forKey: Key.weeklyClean), days(since: Key.lastAutoClean) >= 7 {
            defaults.set(Date(), forKey: Key.lastAutoClean)
            Task {
                if let s = await model.quietClean() {
                    Notifier.post(title: String(localized: "Room for Mac cleaned up"),
                                  body: String(localized: "\(Bytes.string(s.totalSize)) of caches and old files moved to the Trash. Open Room for Mac to see the receipt."))
                }
            }
        }
        ...
        if let s = DiskSpace.current(), s.usedFraction >= 0.9, days(since: Key.lastFullWarning) >= 1 {
            ...
            Notifier.post(title: String(localized: "Your Mac is almost full"),
                          body: String(localized: "Only \(Bytes.string(s.free)) left. Open Room for Mac to make some space."))
        }
        let last = History.shared.lastClean ?? (defaults.object(forKey: Key.firstLaunch) as? Date) ?? Date()
        if Date().timeIntervalSince(last) > 30 * 86400, days(since: Key.lastNudge) >= 7 {
            defaults.set(Date(), forKey: Key.lastNudge)
            Notifier.post(title: String(localized: "Time for a quick clean?"),
                          body: String(localized: "It's been a month. A clean takes about a minute."))
        }
    }
