...
struct SafetyCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Safety check-up"), icon: "lock.shield.fill", color: Room.green,
                ...
                ForEach(checks) { c in
                    HStack(alignment: .top, spacing: 10) {
                        Image(systemName: c.ok == true ? "checkmark.circle.fill" : (c.ok == false ? "exclamationmark.triangle.fill" : "questionmark.circle"))
                            .foregroundStyle(c.ok == true ? Room.green : (c.ok == false ? Room.orange : .secondary))
                            .padding(.top, 2)
...
struct DeviceBatteryCard: View {
    ...
    var body: some View {
        if let devs = model.deviceBatteries, !devs.isEmpty {
            SpeedCard(title: String(localized: "Your devices' batteries"), icon: "airpods", color: Room.blue,
                      subtitle: String(localized: "Headphones, mice, keyboards and trackpads connected over Bluetooth.")) {
                ForEach(devs) { d in
                    HStack(spacing: 10) {
                        Image(systemName: icon(for: d.kind)).foregroundStyle((d.lowest ?? 100) < 20 ? Room.red : Room.blue).frame(width: 22)
                        VStack(alignment: .leading, spacing: 1) {
                        ...
                        HStack(spacing: 10) {
                            ForEach(d.levels, id: \.0) { label, pct in
                                HStack(spacing: 4) {
                                    ...
                                    Text("\(pct)%").font(.callout.monospacedDigit()).foregroundStyle(pct < 20 ? Room.red : .primary)
                                }
                            }
                        }
                        if (d.lowest ?? 100) < 20 { Text("charge soon").font(.caption.weight(.semibold)).foregroundStyle(Room.red) }
                    }
                    .padding(.vertical, 2)
                    .accessibilityElement(children: .combine)
                }
...
struct DefaultAppsCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Which app opens what"), icon: "arrow.up.forward.app", color: Room.purple,
...
struct QuickFixCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Quick fixes"), icon: "wrench.and.screwdriver.fill", color: Room.orange,
