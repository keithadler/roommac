...
//
//  `RoomMac --screenshots <dir>` fills every model with made-up sample data, renders each window
...
//      "build/Room for Mac.app/Contents/MacOS/RoomMac" --screenshots docs/screenshots
...
enum Demo {
    ...
    static func speed() -> SpeedModel {
        ...
        m.agents = [
            AgentInfo(plistPath: "/Users/sam/Library/LaunchAgents/com.example.cloudsync.plist", label: "com.example.cloudsync", program: "", vendor: "Example", loaded: true, runAtLoad: true, system: false, disabledByClean: false, cpu: 0.4, memory: 180_000_000),
            AgentInfo(plistPath: "/Users/sam/Library/LaunchAgents/com.printco.monitor.plist", label: "com.printco.monitor", program: "", vendor: "Printco", loaded: true, runAtLoad: true, system: false, disabledByClean: false, cpu: 0, memory: 22_000_000),
        ]
    ...
    @MainActor
    static func history() {
        func rec(_ n: String, _ mb: Double, kind: CleanKind) -> CleanRecord {
            CleanRecord(id: UUID(), name: n, originalPath: "/Users/sam/Library/Caches/\(n)", trashPath: "/Users/sam/.Trash/\(n)", size: Int64(mb * 1_000_000), restored: false, kind: kind)
        }
        History.shared.sessions = [
            CleanSession(id: UUID(), date: Date().addingTimeInterval(-3600), automatic: false, label: nil,
                        records: [rec("Safari", 1240, kind: .caches), rec("Photo Editor", 612, kind: .caches), rec("PhotoEditor-4.2.dmg", 410, kind: .installers)]),
            CleanSession(id: UUID(), date: Date().addingTimeInterval(-7 * 86400), automatic: true, label: "Quiet clean",
                        records: [rec("Music", 340, kind: .caches), rec("Backup Helper", 96, kind: .logs)]),
            CleanSession(id: UUID(), date: Date().addingTimeInterval(-20 * 86400), automatic: false, label: "Uninstalled Old Video Converter",
    ...
    static func sorter() -> SorterModel {
        ...
        func mv(_ name: String, _ folder: String, _ cat: String, _ mb: Double) -> SortMove {
            SortMove(from: "/Users/sam/\(folder)/\(name)", to: "/Users/sam/\(folder)/Sorted/\(cat)/\(name)", category: cat, size: Int64(mb * 1_000_000))
        }
    ...
    static func menuBar() -> MenuBarModel {
        ...
        m.demo = true
        m.supported = true
        m.apple = MenuBarClean.items.map { MenuBarItemSetting(key: $0.key, name: $0.name, note: $0.note, visible: ["WiFi", "Sound", "Battery", "Bluetooth"].contains($0.key)) }
        m.spotlight = true
...
enum Screenshots {
    ...
    static func render(to dir: String) {
        ...
        let shots: [(String, AnyView, CGSize)] = [
            ("clean", AnyView(CleanupView(model: cleanup)), CGSize(width: 900, height: 860)),
            ...
            ("menubar", AnyView(MenuBarCleanView(model: Demo.menuBar())), CGSize(width: 680, height: 780)),
        ...
        for (name, view, size) in shots {
            if let png = snapshot(view, size: size, title: "Room for Mac") {
    ...
    static func snapshot(_ view: AnyView, size: CGSize, title: String, chrome: Bool = true) -> Data? {
        ...
        guard let frame = chrome ? win.contentView?.superview : win.contentView else { return nil }
        // Always Retina, whatever display this is: bitmapImageRepForCachingDisplay follows the window's
        // backing scale, which is 1 on a machine with no Retina screen (a CI runner), and half-resolution
        // screenshots look soft in the README. Ask for the pixels explicitly instead.
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    func applicationDidFinishLaunching(_ notification: Notification) {
        // The weekly quiet clean and reminders only run while the app is open: register as a login
