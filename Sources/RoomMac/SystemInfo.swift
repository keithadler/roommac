...
enum SystemInfo {
    ...
    static let disabledAgentsDir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        .appendingPathComponent("Room for Mac/Disabled Agents", isDirectory: true)
    ...
    /// and the ones Room for Mac has parked in its Disabled Agents folder. "Loaded" comes from
        ...
        for (dir, system, disabled) in places {
            for u in Scanner.contents(dir) where u.pathExtension == "plist" {
                ...
                out.append(AgentInfo(plistPath: u.path, label: label, program: program, vendor: vendor,
                                     ...
                                     system: system, disabledByClean: disabled,
    ...
    /// Unloads a user launch agent and parks its plist in Room for Mac's folder so it can be re-enabled.
