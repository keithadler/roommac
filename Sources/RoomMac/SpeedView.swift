    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let m = model.message {
                Divider()
                HStack {
                    Image(systemName: "info.circle").foregroundStyle(Room.blue)
                    Text(m).font(.callout)
                    Spacer()
...
func verdictColor(_ v: Verdict) -> Color {
    switch v { case .good: return Room.green; case .watch: return Room.orange; case .act: return Room.red }
}
...
struct RightNowCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "What's using your Mac right now"), icon: "gauge.with.dots.needle.67percent", color: Room.blue,
                ...
                VStack(alignment: .leading, spacing: 4) {
                    GeometryReader { geo in
                        ZStack(alignment: .leading) {
                            ...
                            RoundedRectangle(cornerRadius: 6).fill((m.pressure >= 4 ? Room.red : m.pressure >= 2 ? Room.orange : Room.green).gradient)
            ...
            if let bg = model.background {
                HStack(alignment: .top, spacing: 8) {
                    Image(systemName: "hourglass").foregroundStyle(Room.orange)
                    VStack(alignment: .leading, spacing: 1) {
                ...
                .padding(10).background(Room.orange.opacity(0.1), in: RoundedRectangle(cornerRadius: 8))
            }
            if model.uptimeDays >= 14 {
                HStack(spacing: 8) {
                    Image(systemName: "arrow.counterclockwise.circle").foregroundStyle(Room.purple)
                ...
                .padding(10).background(Room.purple.opacity(0.08), in: RoundedRectangle(cornerRadius: 8))
            }
            ...
            VStack(spacing: 0) {
                ForEach(Array(shown)) { app in
                    HStack(spacing: 10) {
                        ...
                        if app.cpu >= 80 { Text("busy").font(.caption2.weight(.semibold)).padding(.horizontal, 5).padding(.vertical, 1).background(Room.red.opacity(0.15), in: Capsule()).foregroundStyle(Room.red) }
                        Spacer()
...
struct StartupCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Things that start with your Mac"), icon: "power.circle.fill", color: Room.green,
            ...
            if let e = model.loginItemsError {
                HStack(alignment: .top, spacing: 8) {
                    Image(systemName: "hand.raised.fill").foregroundStyle(Room.orange)
                    Text(e).font(.callout)
                    Spacer()
            ...
            if let b = model.boot, b.bootToLoginSeconds != nil || b.loginToReadySeconds != nil {
                ...
                .padding(10).background(Room.green.opacity(0.08), in: RoundedRectangle(cornerRadius: 8))
            }
            ...
            if !user.isEmpty {
                ...
                ForEach(user) { a in
                    HStack(spacing: 10) {
                        Toggle("", isOn: Binding(get: { !a.disabledByClean }, set: { _ in model.toggleAgent(a) }))
...
struct BrowserCard: View {
    ...
    var body: some View {
        if !model.browsers.isEmpty {
            SpeedCard(title: String(localized: "Browser weight"), icon: "globe", color: Room.teal,
                ...
                ForEach(model.browsers) { b in
                    VStack(alignment: .leading, spacing: 6) {
                        HStack {
                            ...
                            if b.running { Text("open").font(.caption2.weight(.semibold)).padding(.horizontal, 5).padding(.vertical, 1).background(Room.blue.opacity(0.15), in: Capsule()).foregroundStyle(Room.blue) }
                            Spacer()
...
struct StorageCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Space macOS can free on its own"), icon: "icloud.and.arrow.up", color: Room.purple,
            ...
            if let s = DiskSpace.current() {
                ...
                HStack(spacing: 8) {
                    Image(systemName: freeFrac < 0.1 ? "exclamationmark.triangle.fill" : "checkmark.circle.fill")
                        .foregroundStyle(freeFrac < 0.1 ? Room.red : Room.green)
                    Text(freeFrac < 0.1
                         ? "Only \(Int(freeFrac * 100))% free. Below the 10% line, so use Clean Up first."
    ...
    private func row(_ title: LocalizedStringKey, _ detail: LocalizedStringKey, _ pane: String?) -> some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "switch.2").foregroundStyle(Room.purple).padding(.top, 2).accessibilityHidden(true)
            VStack(alignment: .leading, spacing: 1) {
...
struct UpdatesCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Updates you've been putting off"), icon: "arrow.down.circle.fill", color: Room.orange,
            ...
            HStack(spacing: 10) {
                if model.checkingUpdates {
                    ...
                    if u.isEmpty {
                        Label("macOS is up to date", systemImage: "checkmark.circle.fill").foregroundStyle(Room.green)
                    } else {
                        VStack(alignment: .leading, spacing: 2) {
...
struct LighterLookCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "A lighter look for older Macs"), icon: "sparkle", color: Room.pink,
    ...
    private func status(_ label: LocalizedStringKey, _ on: Bool?) -> some View {
        HStack(spacing: 8) {
            Image(systemName: on == true ? "checkmark.circle.fill" : "circle").foregroundStyle(on == true ? Room.green : .secondary)
            Text(label)
            Spacer()
