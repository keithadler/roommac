...
final class UninstallModel {
    ...
    func uninstall() {
        ...
        Task {
            ...
            records.append(CleanRecord(id: UUID(), name: app.name, originalPath: app.path, trashPath: appTrash, size: appSize, restored: false, kind: nil))
            done += 1; phase = .working(done: done, total: paths.count)
            for item in items {
                for p in item.paths {
                    let t = await Task.detached { Scanner.moveToTrash(p) }.value
                    records.append(CleanRecord(id: UUID(), name: item.name, originalPath: p, trashPath: t, size: item.size, restored: false, kind: .leftovers))
                    done += 1; phase = .working(done: done, total: paths.count)
                }
            }
            let session = CleanSession(id: UUID(), date: Date(), automatic: false,
                                      label: String(localized: "Uninstalled \(app.name)"), records: records)
            History.shared.add(session)
            if appTrash == nil {
                phase = .failed(String(localized: "macOS wouldn't let Room for Mac move \(app.name) to the Trash. It may need an administrator, or it may be managed by your school or workplace."))
            } else {
    ...
    private var detail: some View {
        if let app = model.selected {
            VStack(alignment: .leading, spacing: 14) {
                ...
                case .done(let moved, let size, let failed):
                    VStack(alignment: .leading, spacing: 8) {
                        Label("\(app.name) is gone", systemImage: "checkmark.circle.fill").font(.title2.weight(.semibold)).foregroundStyle(Room.green)
                        Text("\(moved) items moved to the Trash, \(Bytes.string(size)). Empty the Trash to free the space, or use What Room for Mac did to put it back.")
                        if failed > 0 { Text("\(failed) items couldn't be moved because macOS protects them.").foregroundStyle(.secondary) }
                        Button("Done") { model.reset() }.buttonStyle(.borderedProminent).tint(Room.green).controlSize(.large)
                    }
                    Spacer()
                case .failed(let msg):
                    Label(msg, systemImage: "exclamationmark.triangle.fill").foregroundStyle(Room.orange)
                ...
                default:
                                ...
                                VStack(alignment: .leading, spacing: 1) {
                                    HStack(spacing: 6) {
                                        ...
                                        if let p = item.protectedBy {
                                            Label("Protected: \(p)", systemImage: "shield.fill").font(.caption).foregroundStyle(Room.blue)
                                        }
                                    }
                    ...
                    Text("Everything goes to the Trash, and a receipt is saved under What Room for Mac did.")
            ...
            ContentUnavailableView("Pick an app", systemImage: "trash.slash",
                                   description: Text("Choose an app on the left, or drag one in from Finder. Room for Mac finds everything it left behind and removes it all together."))
        }
    }
}
