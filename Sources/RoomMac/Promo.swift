//  Announcement images (docs/promo). Rendered by `roommac screenshots <dir>` alongside the README
...
enum Promo {
                ...
                HStack(spacing: 40) {
                    VStack(alignment: .leading, spacing: 22) {
                        HStack(spacing: 18) {
                            ...
                            Text("Room for Mac").font(.system(size: 64, weight: .bold)).foregroundStyle(.white)
                        }
    ...
    // 3. Three tabs
    struct Tabs: View {
        let clean: NSImage?, speed: NSImage?, money: NSImage?
        var body: some View {
            ZStack {
                gradient
                VStack(spacing: 26) {
                    ...
                    HStack(alignment: .top, spacing: 30) {
                        column("Clean", "What's safe to clear, and why", clean)
            ...
            ZStack {
                ...
                HStack(spacing: 50) {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        $ roommac up --safe --dry-run
                        Would move 10 items (4.77 GB) to the Trash. Nothing was changed.

                        $ roommac speed --json | jq .verdict
                        "green"

                        $ roommac restore last
                        Put back 10 items.
                        """)
    ...
    static func render(into dir: URL, shots: URL) {
        ...
        let cards: [(String, AnyView)] = [
            ("1-hero", AnyView(Hero(clean: img("clean")))),
            ("2-claude", AnyView(ClaudeCard(model: claude))),
            ("3-tabs", AnyView(Tabs(clean: img("clean"), speed: img("speed"), money: img("money")))),
