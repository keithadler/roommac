//  The Clean tab UI: disk gauge header (with the 10%-free guardrail), one CategoryCard per
//  CleanKind, an ItemRow per file with checkbox / thumbnail / shield / "why" popover, the footer
//  with the big Clean Up button, and the confirmation sheets. Also defines the Clean palette and
    ...
    var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItemGroup(placement: .primaryAction) {
                Button { openWindow(id: "history") } label: { Label("What Room for Mac did", systemImage: "clock.arrow.circlepath") }
                    .help("See everything Room for Mac has moved, and put any of it back")
                ...
                SettingsLink { Label("Settings", systemImage: "gearshape") }
                    .help("Menu bar, weekly clean, reminders, protected folders")
            }
        }
            ...
            LazyVStack(spacing: compact ? 8 : 14) {
                ...
                if model.isScanning {
                    ...
                    ContentUnavailableView("Sparkling clean!", systemImage: "sparkles",
                                           description: Text("There's nothing to clean up right now."))
                        .padding(.top, 40)
                }
            }
...
struct Header: View {
    ...
    private var headline: LocalizedStringKey {
        guard let s = model.space else { return "Let's clean up." }
        if s.usedFraction < 0.7 { return "Plenty of room. A quick clean keeps things zippy." }
    ...
    private var gaugeColor: Color {
        ...
        if s.usedFraction < 0.7 { return Room.green }
        if s.usedFraction < 0.9 { return Room.orange }
        return Room.red
    }
            ...
            HStack(alignment: .top) {
                HStack(spacing: 12) {
                    Image(systemName: "sparkles")
                        ...
                        .foregroundStyle(Room.green)
                        .accessibilityHidden(true)
                    if compact {
                        Text("Room for Mac").font(.title2.weight(.bold))
                        Text(headline).font(.callout).foregroundStyle(.secondary)
                    } else {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Room for Mac").font(.largeTitle.weight(.bold))
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    HStack {
                        ...
                        if s.usedFraction > 0.9 {
                            Text("· past the 10% line, macOS will feel slow").foregroundStyle(Room.red).fontWeight(.semibold)
                        }
                        Spacer()
...
struct CategoryCard: View {
    ...
    private var color: Color { Room.color(for: kind) }
    private var safetyColor: Color {
        switch kind.safety { case .safe: return Room.green; case .copies: return Room.blue; case .mixed: return Room.teal; case .review: return Room.orange }
    }
                ...
                HStack(spacing: 6) {
                    ...
                    if let why = item.protectedBy {
                        Label("Protected: \(why)", systemImage: "shield.fill")
                            .font(.caption.weight(.semibold)).foregroundStyle(Room.blue)
                            .padding(.horizontal, 6).padding(.vertical, 2)
                            .background(Room.blue.opacity(0.12), in: Capsule())
                    }
                }
...
struct Footer: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            HStack(alignment: .center, spacing: 16) {
                VStack(alignment: .leading, spacing: 2) {
                    if model.selectedCount > 0 {
                        Text("Ready to clean: \(Bytes.string(model.selectedTotal))").font(.title3.weight(.semibold))
                        ...
                        Text("Tick the boxes above to choose what to clean.").foregroundStyle(.secondary)
                    }
                }
                Spacer()
                ...
                .buttonStyle(.borderedProminent).tint(Room.green).controlSize(.large)
            ...
            if !compact {
                HStack(spacing: 6) {
                    Image(systemName: "checkmark.shield.fill").foregroundStyle(Room.green).accessibilityHidden(true)
                    Text("Everything goes to the Trash first, so you can change your mind. Room for Mac never touches your photos, documents, or apps.")
                        .font(.caption).foregroundStyle(.secondary)
                    Spacer()
                }
            }
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            case .cleaning(let done, let total):
                Image(systemName: "sparkles").font(.system(size: 40)).foregroundStyle(Room.green)
                Text("Cleaning up…").font(.title2.weight(.semibold))
            ...
            case .finished(let moved, let size, let failed):
                Image(systemName: "checkmark.circle.fill").font(.system(size: 44)).foregroundStyle(Room.green)
                ...
                Text("The space isn't free yet. Empty the Trash when you're sure you don't need any of it. A receipt is saved under What Room for Mac did, so anything can be put back.")
            ...
            default:
                Image(systemName: "trash").font(.system(size: 40)).foregroundStyle(Room.blue)
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ForEach(model.categories.filter { $0.selectedCount > 0 }) { c in
                        HStack {
                            Circle().fill(Room.color(for: c.kind)).frame(width: 8, height: 8)
                            Text(c.kind.title)
                            Spacer()
                ...
                Text("Nothing is deleted. You can drag anything back out of the Trash, or use What Room for Mac did to put it back with one click.")
                ...
                HStack {
                    ...
                    Button("Move to Trash") { model.clean() }
                        .buttonStyle(.borderedProminent).tint(Room.green).controlSize(.large).keyboardShortcut(.defaultAction)
                }
            }
        }
        .padding(30)
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            case .trashFailed(let msg):
                Image(systemName: "exclamationmark.triangle.fill").font(.system(size: 40)).foregroundStyle(Room.orange)
                Text("Couldn't empty the Trash").font(.title2.weight(.semibold))
                Text("macOS didn't let Room for Mac ask Finder to do it. You can empty it yourself: click the Trash in the Dock, then choose Empty.")
            ...
            default:
                Image(systemName: "trash.fill").font(.system(size: 40)).foregroundStyle(Room.red)
                ...
                HStack {
                    ...
                    Button("Empty Trash") { model.emptyTrash() }
                        .buttonStyle(.borderedProminent).tint(Room.red).controlSize(.large)
                }
            }
        }
        .padding(30)
