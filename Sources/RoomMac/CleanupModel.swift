...
//  receipt) and emptyTrash() (via Finder). quietClean() is the unattended weekly variant and only
...
enum CleanKind: String, CaseIterable, Sendable, Identifiable, Codable {
    ...
    /// The kinds a quiet, unattended clean is allowed to touch.
...
final class CleanupModel {
    ...
    var lastClean: Date? { History.shared.lastClean }
        ...
        Task {
            ...
            for item in items {
                ...
                for p in item.paths {
                    ...
                    records.append(CleanRecord(id: UUID(), name: item.name, originalPath: p, trashPath: trashPath,
                                              size: per, restored: false, kind: kindOf[item.id]))
                    done += 1
                    phase = .cleaning(done: done, total: total)
                }
            }
            let session = CleanSession(id: UUID(), date: Date(), automatic: false, label: nil, records: records)
    ...
    /// Unattended clean of the always-safe categories only. Used by the weekly schedule and the menu bar.
    func quietClean() async -> CleanSession? {
        guard !quietCleanRunning, !isScanning else { return nil }
        quietCleanRunning = true
        defer { quietCleanRunning = false }
        var records: [CleanRecord] = []
        ...
        let session = CleanSession(id: UUID(), date: Date(), automatic: true, label: String(localized: "Quiet clean"), records: records)
        History.shared.add(session)
        space = DiskSpace.current()
        trash = await Task.detached { Scanner.trashState() }.value
        return session
