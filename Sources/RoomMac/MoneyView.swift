    ...
    var body: some View {
        VStack(spacing: 0) {
            ScrollView {
                LazyVStack(spacing: compact ? 8 : 14) {
                    ...
                    HStack(spacing: 6) {
                        ...
                        Text("Prices are typical US figures for 2026 and change often. Room for Mac can't see what you actually pay, so every saving here is an \"if you're paying for this\".")
            ...
            if let m = model.message {
                Divider()
                HStack { Image(systemName: "info.circle").foregroundStyle(Room.blue); Text(m).font(.callout); Spacer(); Button("OK") { model.message = nil }.controlSize(.small) }
...
struct SubscriptionsCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Subscriptions you may not be using"), icon: "creditcard.fill", color: Room.green,
                  subtitle: String(localized: "Apps that are normally paid monthly or yearly, and when each was last opened. Room can't see your bills, so treat these as questions, not accusations.")) {
            if let subs = model.subscriptions {
                if subs.isEmpty { Label("No subscription apps found.", systemImage: "checkmark.circle.fill").foregroundStyle(Room.green) }
                ForEach(subs) { s in
                            ...
                            HStack(spacing: 6) {
                                ...
                                if idle >= 60 {
                                    Text(s.lastUsed == nil ? "never opened" : "not opened in \(idle) days")
                                        ...
                                        .background(Room.orange.opacity(0.15), in: Capsule()).foregroundStyle(Room.orange)
                                } else {
...
struct InternetPlanCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Are you getting the internet you pay for?"), icon: "gauge.with.needle", color: Room.blue,
            ...
            if let t = model.speedTest {
                ...
                if plan > 0 {
                    ...
                    if ratio < 0.5 {
                        tip(Room.orange, "That's under half of what you're paying for. Test again with a cable; if it's still low, call your provider: they'll often fix it or drop you to the cheaper plan you're actually getting.")
                    } else if ratio < 0.8 {
                        tip(.secondary, "A bit under the plan speed, which is normal on Wi-Fi. Fine.")
                    } else {
                        tip(Room.green, "You're getting what you pay for.")
                    }
...
struct StorageMathCard: View {
    ...
    var body: some View {
        SpeedCard(title: String(localized: "Before you buy more storage"), icon: "externaldrive.badge.plus", color: Room.purple,
                  subtitle: String(localized: "Storage upgrades are the most common thing people pay for that a clean would have covered.")) {
            if let s = model.storage {
                if let sp = s.space {
                    Text("\(Bytes.string(sp.free)) free now. Room can free about \(Bytes.string(s.safeBytes)) safely, and another \(Bytes.string(s.reviewBytes)) if you go through the \"take a look first\" cards.")
                        .font(.callout)
                    let after = sp.free + s.safeBytes + s.reviewBytes
                    tip(after > sp.total / 5 ? Room.green : Room.orange,
                        after > sp.total / 5
                        ? "After a full clean you'd have \(Bytes.string(after)) free, a fifth of the disk. No need to buy anything."
                ...
                HStack { ProgressView().controlSize(.small); Text("Adding up what a clean would free…").foregroundStyle(.secondary) }
            }
        }
    }
}
...
struct CloudDrivesCard: View {
    ...
    var body: some View {
        if let clouds = model.clouds {
            ...
            SpeedCard(title: String(localized: "Cloud drives"), icon: "icloud.fill", color: Room.teal,
                ...
                ForEach(clouds) { c in
                    HStack(spacing: 10) {
                        Image(systemName: c.running ? "checkmark.icloud.fill" : "icloud").foregroundStyle(c.running ? Room.teal : .secondary)
                        VStack(alignment: .leading, spacing: 1) {
                ...
                if active.count >= 2 {
                    tip(Room.orange, "Pick the one that comes with something you already pay for (iCloud with an iPhone, Google One with Gmail, OneDrive with Microsoft 365), move the files over, and cancel the rest.")
                }
            }
        }
    }
...
struct AlternativesCard: View {
    ...
    var body: some View {
        if let alts = model.alternatives, !alts.isEmpty {
            SpeedCard(title: String(localized: "Things your Mac already does for free"), icon: "gift.fill", color: Room.pink,
