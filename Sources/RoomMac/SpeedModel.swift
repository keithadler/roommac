...
final class SpeedModel {
    ...
    var findings: [Finding] {
        ...
        if let s = space {
            ...
            if freeFrac < 0.05 { f.append(Finding(level: .act, title: String(localized: "Disk almost full"), detail: String(localized: "Only \(Bytes.string(s.free)) free. macOS slows down badly below 5%. Run Clean Up."))) }
    ...
    func refreshSlow() {
        ...
        Task {
            ...
            loginItems = r.items
            loginItemsError = r.denied ? String(localized: "Room for Mac isn't allowed to read login items. Allow it under Privacy & Security > Automation > Room for Mac > System Events.") : nil
    ...
    func removePrinter(_ p: PrinterInfo) {
        ...
        History.shared.add(CleanSession(id: UUID(), date: Date(), automatic: false, label: String(localized: "Removed printer \(p.name)"), records: []))
        Task { printers = await Task.detached { SystemInfo2.printers() }.value }
    }
    ...
    func removeLoginItem(_ item: LoginItem) {
        ...
        History.shared.add(CleanSession(id: UUID(), date: Date(), automatic: false,
    ...
    func toggleAgent(_ a: AgentInfo) {
        let err = a.disabledByClean ? SystemInfo.enableAgent(a) : SystemInfo.disableAgent(a)
        if let err { message = err; return }
        if !a.disabledByClean {
            History.shared.add(CleanSession(id: UUID(), date: Date(), automatic: false,
                                           label: String(localized: "Turned off background helper \(a.vendor)"),
                                           records: [CleanRecord(id: UUID(), name: a.label, originalPath: a.plistPath,
        ...
        message = a.disabledByClean ? String(localized: "\(a.vendor) helper turned back on.") : String(localized: "\(a.vendor) helper turned off. It won't start at next login. Turn it back on here any time.")
        Task {
            let procs = await Task.detached { SystemInfo.processes() }.value
    ...
    func clearBrowserCaches(_ b: BrowserInfo) {
        ...
        Task {
            ...
            for p in paths {
                ...
                records.append(CleanRecord(id: UUID(), name: URL(fileURLWithPath: p).lastPathComponent, originalPath: p, trashPath: t, size: size, restored: false, kind: .caches))
            }
            let s = CleanSession(id: UUID(), date: Date(), automatic: false, label: String(localized: "Cleared \(b.name) site data"), records: records)
