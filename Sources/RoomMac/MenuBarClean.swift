//  "Clean the menu bar" window. Apple's icons are toggled by writing the same Control Center
//  preferences System Settings writes (see MenuBarClean.set for the three key forms macOS has
...
final class MenuBarModel {
    ...
    var demo = false
    var supported = MenuBarClean.supported
    var spotlight = true
    var apps: [AppLoad] = []
    ...
    func reload() {
        supported = MenuBarClean.supported
        apple = MenuBarClean.read()
        spotlight = MenuBarClean.spotlightVisible
        refreshApps()
    ...
    func toggle(_ item: MenuBarItemSetting) {
        ...
        if let e = MenuBarClean.set(item.key, visible: on) { message = e; return }
    ...
    func toggleSpotlight() {
        let on = !spotlight
        if let e = MenuBarClean.setSpotlight(visible: on) { message = e; return }
        spotlight = on
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Clean the menu bar").font(.title.weight(.bold))
                    ...
                    GroupBox {
                        HStack(alignment: .top, spacing: 12) {
                            Image(systemName: "command").font(.title).foregroundStyle(Room.blue).frame(width: 36)
                            VStack(alignment: .leading, spacing: 4) {
                                Text("To move an icon: hold ⌘ and drag it").font(.headline)
                                Text("Works for every icon, Apple's and other apps'. Drag an Apple icon right off the bar to remove it. Room for Mac can't reach across and drag for you, but that's all there is to it.")
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            if !model.supported {
                                HStack(alignment: .top, spacing: 8) {
                                    ...
                                    Text("This version of macOS (\(Capabilities.osString)) keeps these switches somewhere Room for Mac can't reach yet. Use the Control Center settings button below; it's the same list.")
                                        .font(.callout)
                                }
                                .padding(8).background(Room.blue.opacity(0.08), in: RoundedRectangle(cornerRadius: 8))
                            }
                            ...
                            HStack(spacing: 10) {
                                Toggle("", isOn: $roomMacIcon).toggleStyle(.switch).labelsHidden().controlSize(.small)
                                    .accessibilityLabel(Text("Show Room for Mac in the menu bar"))
                                VStack(alignment: .leading, spacing: 1) {
                                    Text("Room for Mac")
                                    Text("The sparkle. Reminders and the weekly clean need it, but it's your call.").font(.caption).foregroundStyle(.secondary)
                                }
                                Spacer()
                            }
                    ...
                    GroupBox {
                        HStack(alignment: .top, spacing: 12) {
                            Image(systemName: "eye.slash").font(.title2).foregroundStyle(Room.purple).frame(width: 36)
                            VStack(alignment: .leading, spacing: 4) {
                                Text("Want icons tucked away but still reachable?").font(.headline)
                                Text("macOS has no built-in way to collapse other apps' icons. Ice is a free, open-source app that adds a hidden section to the menu bar. Room for Mac doesn't bundle it, so this is just a pointer.")
                                    .font(.callout).foregroundStyle(.secondary)
