//  Receipts. Every session that moves anything (Clean Up, quiet clean, uninstall, agent disable)
//  records each path with where it landed in the Trash, persisted as JSON under
//  ~/Library/Application Support/Room Mac/history.json. restore() moves a record back to its
//  original path, which works until the Trash is emptied.

import Foundation
import Observation

/// One file (or folder) Room for Mac moved to the Trash.
struct CleanRecord: Codable, Identifiable, Sendable, Hashable {
    var id: UUID
    var name: String
...
/// One clean session: a click of Clean Up, a quiet weekly clean, or an uninstall.
struct CleanSession: Codable, Identifiable, Sendable, Hashable {
    var id: UUID
    var date: Date
    var automatic: Bool
    var label: String?
...
/// Persistent receipt of everything Room for Mac has ever done, so any of it can be put back.
@MainActor
@Observable
final class History {
    ...
    private static let dir: URL = {
        ...
        return base.appendingPathComponent("Room Mac", isDirectory: true)
    }()
    private static let file = dir.appendingPathComponent("history.json")

    init() {
        if let data = try? Data(contentsOf: Self.file),
           let s = try? JSONDecoder().decode([CleanSession].self, from: data) {
            sessions = s.sorted { $0.date > $1.date }
        }
    }

    var lastClean: Date? { sessions.first?.date }

    func add(_ session: CleanSession) {
    ...
    func restore(_ record: CleanRecord, in session: CleanSession) async -> RestoreOutcome {
    ...
    func restoreAll(in session: CleanSession) async -> (restored: Int, gone: Int, failed: Int) {
        var r = 0, g = 0, f = 0
    ...
    func restoreAllSync(in session: CleanSession) -> (restored: Int, gone: Int, failed: Int) {
        var r = 0, g = 0, f = 0
    ...
    nonisolated private static func restoreOnDisk(_ r: CleanRecord) -> RestoreOutcome {
