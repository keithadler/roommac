//  File-system scanning for the Clean tab. Everything here is a pure function that runs off the
...
enum Scanner {
    ...
    static func scanLeftovers() -> [CleanItem] {
        ...
        let skip: Set<String> = ["addressbook", "callhistorydb", "callhistorytransactions", "clouddocs", "crashreporter", "caches",
                                 ...
                                 "ilifemediabrowser", "keychain", "spotlight", "quick look", "sharedfilelist", "icdd", "networkextension", "clean mac"]
    ...
    static func scanBigFiles() -> [CleanItem] {
        ...
        return found.sorted { $0.1 > $1.1 }.prefix(25).map { f, s, d in
            CleanItem(
                ...
                why: String(localized: "This is one of the largest files in your home folder. Room for Mac can't know whether it matters to you, so it's only listed here to help you decide. Use the magnifying glass to see it in Finder first."))
        }
    }
    ...
    static func emptyTrash() -> String? {
        ...
        if Capabilities.automationDenied(err) {
            return String(localized: "Room for Mac isn't allowed to control Finder. Allow it under System Settings > Privacy & Security > Automation, or empty the Trash from the Dock.")
        }
