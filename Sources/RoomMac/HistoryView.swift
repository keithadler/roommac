//  "What Room for Mac did": the receipts window. One section per session, a Put Back button per
//  record, and a Put Everything Back button per session.

import SwiftUI

/// "What Room for Mac did": every session, every item, and a Put Back button for each.
        ...
        VStack(spacing: 0) {
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text("What Room for Mac did").font(.title.weight(.bold))
                    Text("Everything Room for Mac has moved to the Trash. Anything still in the Trash can be put back exactly where it was.")
            ...
            if history.sessions.isEmpty {
                ContentUnavailableView("Nothing yet", systemImage: "clock.arrow.circlepath",
                                       description: Text("Receipts appear here after your first clean."))
            } else {
                List {
...
struct SessionSection: View {
    ...
    private var title: String {
        ...
        return session.automatic ? String(localized: "Quiet clean") : String(localized: "Clean Up")
    }
            ...
            DisclosureGroup(isExpanded: $expanded) {
                ForEach(session.records) { r in
                    HStack(spacing: 10) {
                        Image(systemName: r.restored ? "arrow.uturn.backward.circle.fill" : (r.moved ? "trash" : "xmark.circle"))
                            .foregroundStyle(r.restored ? Room.green : (r.moved ? .secondary : Room.red))
                        ...
                        if r.restored {
                            Text("Put back").font(.caption.weight(.semibold)).foregroundStyle(Room.green)
                        } else if r.moved {
                            Button("Put back") { restore(r) }.controlSize(.small).disabled(busy)
                                .accessibilityLabel(Text("Put back \(r.name)"))
                        } else {
                            Text("Not moved").font(.caption).foregroundStyle(Room.red)
                        }
                    }
                    .padding(.vertical, 2)
                }
                ...
                HStack(spacing: 10) {
                    Image(systemName: session.automatic ? "moon.zzz.fill" : "sparkles")
                        .foregroundStyle(session.automatic ? Room.purple : Room.green)
                        .accessibilityHidden(true)
                    VStack(alignment: .leading, spacing: 1) {
    ...
    private func restore(_ r: CleanRecord) {
        busy = true
        Task {
