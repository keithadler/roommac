...
//  (Screenshots, Images, Documents, Installers, Archives, Videos, Music, Other) under a "Sorted"
//  folder in the same place. Files are moved, not trashed, and every move is written to a receipt,
//  so "put back" in What Room for Mac did restores the original layout exactly.
...
enum Sorter {
    ...
    static func plan(folders: [URL], olderThanDays: Int) -> [SortMove] {
        ...
        for folder in folders {
            for u in Scanner.contents(folder) {
                ...
                if name.hasPrefix(".") || name == "Sorted" || u.pathExtension == "download" || u.pathExtension == "crdownload" || u.pathExtension == "part" { continue }
                ...
                let dest = folder.appendingPathComponent("Sorted/\(cat)/\(name)")
    ...
    static func execute(_ moves: [SortMove]) -> CleanSession {
        ...
        for m in moves {
            ...
            records.append(CleanRecord(id: UUID(), name: URL(fileURLWithPath: m.from).lastPathComponent, originalPath: m.from,
                                      trashPath: ok ? dest.path : nil, size: m.size, restored: false, kind: nil))
        }
        return CleanSession(id: UUID(), date: Date(), automatic: false, label: String(localized: "Sorted Desktop & Downloads"), records: records)
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("Loose files get filed into folders by type inside a \"Sorted\" folder, right where they are. Nothing leaves the Desktop or Downloads, nothing is deleted, and What Room for Mac did can put every file back exactly where it was.")
            ...
            if let s = model.done {
                VStack(spacing: 10) {
                    Image(systemName: "checkmark.circle.fill").font(.system(size: 40)).foregroundStyle(Room.green)
                    Text("Filed \(s.movedCount) files").font(.title2.weight(.semibold))
                    Text("They're in the Sorted folders on your Desktop and in Downloads. To undo, open What Room for Mac did and choose Put everything back.").foregroundStyle(.secondary).multilineTextAlignment(.center).frame(maxWidth: 420)
